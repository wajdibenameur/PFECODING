...
public class ZabbixAdapter {
    ...
    private static final String SOURCE = MonitoringConstants.SOURCE_ZABBIX;
    ...
    public List<ServiceStatusDTO> fetchAll() {
        JsonNode hosts = getSnapshotOrApi("hosts", "hosts", zabbixClient::getHosts);
    ...
    public List<ZabbixProblemDTO> fetchProblems() {
        return fetchProblems(getSnapshotOrApi("hosts", "hosts", zabbixClient::getHosts));
    }

    public List<ZabbixProblemDTO> fetchProblems(JsonNode hosts) {
        Map<String, JsonNode> hostMapById = buildHostMap(hosts);

        JsonNode problemsJson = getSnapshotOrApi(
                "recent problems",
                "recent problems",
                zabbixClient::getRecentProblems
        );
        ...
        for (JsonNode node : problemsJson) {
            ...
            if (fullHost == null && hostId != null) {
                String resolvedHostId = hostId;
                JsonNode hostById = getSnapshotOrApi(
                        "host by id",
                        buildHostByIdSnapshotKey(resolvedHostId),
                        () -> zabbixClient.getHostById(resolvedHostId)
                );
    ...
    private JsonNode fetchTriggerById(String triggerId) {
        ...
        return getSnapshotOrApi(
                "trigger by id",
                buildTriggerByIdSnapshotKey(triggerId),
                () -> zabbixClient.getTriggerById(triggerId)
        );
    }
    ...
    public List<ZabbixMetricDTO> fetchMetricsAndMap() {
        return fetchMetricsAndMap(getSnapshotOrApi("hosts", "hosts", zabbixClient::getHosts));
    }
    ...
    public List<ZabbixMetricDTO> fetchMetricsAndMap(JsonNode hosts) {
        ...
        JsonNode items = getSnapshotOrApi(
                "items by hosts",
                buildItemsByHostsSnapshotKey(hostIds),
                () -> zabbixClient.getItemsByHosts(hostIds)
        );
        ...
        if (!floatItems.isEmpty()) {
            for (List<String> batch : chunkList(floatItems, HISTORY_BATCH_SIZE)) {
                try {
                    JsonNode history = getSnapshotOrApi(
                            "history batch",
                            buildHistoryBatchSnapshotKey(batch, 0, windowStart, now),
                            () -> zabbixClient.getHistoryBatch(batch, 0, windowStart, now)
                    );
        ...
        if (!intItems.isEmpty()) {
            for (List<String> batch : chunkList(intItems, HISTORY_BATCH_SIZE)) {
                try {
                    JsonNode history = getSnapshotOrApi(
                            "history batch",
                            buildHistoryBatchSnapshotKey(batch, 3, windowStart, now),
                            () -> zabbixClient.getHistoryBatch(batch, 3, windowStart, now)
                    );
                    mapHistory(metrics, history, itemKeyMap, itemHostMap, hostNames, hostMap);
                } catch (RuntimeException e) {
    ...
    private JsonNode getSnapshotOrApi(String context, String snapshotKey, Supplier<JsonNode> apiCall) {
        try {
            JsonNode liveData = apiCall.get();
            if (liveData != null) {
                return liveData;
            }
            log.warn(LOG_PREFIX + "Live Zabbix call returned null for '{}', attempting Redis fallback", context);
        } catch (RuntimeException exception) {
            log.warn(LOG_PREFIX + "Live Zabbix call failed for '{}': {}", context, exception.getMessage());
        }

        Optional<JsonNode> cachedSnapshot = getSnapshot(snapshotKey);

        if (cachedSnapshot.isPresent()) {
            log.warn(LOG_PREFIX + "Using Redis fallback snapshot for '{}'", context);
            availabilityService.markUnavailable(SOURCE, "Zabbix API unavailable, serving Redis snapshot for " + context);
            return cachedSnapshot.get();
        }

        log.warn(LOG_PREFIX + "No usable Redis snapshot for '{}', keeping current fallback behavior", context);
        availabilityService.markUnavailable(SOURCE, "Zabbix API unavailable and no usable Redis snapshot for " + context);
        return null;
    }

    private Optional<JsonNode> getSnapshot(String snapshotKey) {
        return integrationCacheService.getSnapshot(
                SOURCE,
    ...
    private String joinParts(List<String> values) {
        ...
        return String.join(",", values.stream().map(this::safePart).toList());
    }
